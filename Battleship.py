...
class Taulell():
    ...
    def controla_coordenada(self, coordenades, tipus, orientacio): 
            try:
...
class Partida():
    ...
    def Comenca_partida(self):
        print("Vols inicialitzar el teu taulell de forma manual (m) o de forma random (r)?")
        mode = input("Introdueix 'm' per manual o 'r' per random: ").lower()
        while mode not in ['m', 'r']:
            print("Opció no vàlida. Si us plau, introdueix 'm' o 'r'.")
            mode = input("Introdueix 'm' per manual o 'r' per random: ").lower()
        if mode == 'm':
            self.jugador1.inicialitza_taulell()
        else:
            self.jugador1.taulell.inicialitza_random()
